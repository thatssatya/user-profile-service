...
public class DefaultHttpClient implements HttpClient, Closeable {
    ...
    public <R> CompletableFuture<R> executeAsync(ApiRequest request, Class<R> responseCls) {
        return executeAsyncInternal(request, responseCls);
    }

    @Override
    public <R> R execute(ApiRequest request, Class<R> responseCls) {
        return executeAsyncInternal(request, responseCls).join();
    }

    private <R> CompletableFuture<R> executeAsyncInternal(ApiRequest request, Class<R> responseCls) {
    ...
    private HttpUriRequest buildHttpUriRequest(ApiRequest request) {
        var httpRequest = getHttpRequestBase(request);
        request.getHeaders().forEach(httpRequest::setHeader);
        return httpRequest;
    }

    private HttpRequestBase getHttpRequestBase(ApiRequest request) {
